    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="fangjianCollectionTypes">
                        <el-select v-model="ruleForm.fangjianCollectionTypes" :disabled="ro.fangjianCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in fangjianCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="fangjianCollectionValue">
                        <el-input v-model="ruleForm.fangjianCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    fangjianId: false,
                    yonghuId: false,
                    fangjianCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    fangjianId: '',
                    yonghuId: '',
                    fangjianCollectionTypes: '',
                    insertTime: '',
                },
                fangjianCollectionTypesOptions : [],
                fangjianOptions : [],
                yonghuOptions : [],
                rules: {
                   fangjianId: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=fangjian_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.fangjianCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `fangjianCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`fangjianCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.fangjianCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
